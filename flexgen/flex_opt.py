...
from flexgen.llama_config import LlamaConfig, get_llama_config
...
class LlamaGroupedQueryAttention:
    def __init__(self, config, env, policy, layer_id):
        self.config = config
        self.env = env
        self.layer_id = layer_id
...
class LlamaRMSNorm(torch.nn.Module):
    def __init__(self, config, env, policy, layer_id, hidden_size, eps=1e-6):
        """
...
class Llama3wayMLP:
    def __init__(self, config, env, policy, layer_id):
        ...
        self.task = None
        self.post_attetion_layernorm = LlamaRMSNorm(config, env, policy, layer_id, self.config.hidden_size, eps=self.config.layer_norm_eps)
...
class TransformerLayer_llama:
    def __init__(self, config, env, policy, i):
        self.attention = LlamaGroupedQueryAttention(config, env, policy, i)
        self.mlp = Llama3wayMLP(config, env, policy, i)
        self.policy = policy
        self.compute = self.attention.compute
    ...
    def __init__(self,
        ...
        for i in range(self.config.num_hidden_layers):
            if policy.sep_layer:
                ...
                layers.append(TransformerLayer_llama(self.config, self.env, self.policy, i))
